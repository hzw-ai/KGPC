...
data = []
for p in files:
    with open(join(base_path, p), encoding='utf-8') as f:
...
for p in files:
    with open(join(base_path, p), encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            # if e2 not in label_graph[(e1, rel)]:
            #     label_graph[(e1, rel)].append(e2)
            # if e1 not in label_graph[(e2, rel_reverse)]:
            ...
            # if e2 not in train_graph[p][(e1, rel)]:
            #     train_graph[p][(e1, rel)].append(e2)
            # if e1 not in train_graph[p][(e2, rel_reverse)]:
